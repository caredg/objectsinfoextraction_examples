...
void 
MuonObjectInfoExtractor::analyzeMuons(const edm::Event& iEvent, const edm::Handle<reco::MuonCollection> &muons)
{
  ...
  if(muons.isValid()){
	...
	for (reco::MuonCollection::const_iterator recoMu = muons->begin(); recoMu!=muons->end(); ++recoMu){
	  ...
	  //here one could apply some cuts maybe to show particle id, and store
	  //refined variables
	}
      else{
	mu_e.push_back(-999);
